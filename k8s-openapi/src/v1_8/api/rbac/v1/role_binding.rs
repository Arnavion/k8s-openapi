...
// Generated from operation watchRbacAuthorizationV1RoleBindingForAllNamespacesList
...
impl RoleBinding {
    ...
    /// This operation only supports watching a list of items for changes.
    ///
    /// Use the returned [`crate::ResponseBody`]`<`[`WatchRoleBindingForAllNamespacesListResponse`]`>` constructor, or [`WatchRoleBindingForAllNamespacesListResponse`] directly, to parse the HTTP response.
    ///
    /// # Arguments
    ///
    /// * `optional`
    ///
    ///     Optional parameters. Use `Default::default()` to not pass any.
    pub fn watch_role_binding_for_all_namespaces_list(
        optional: WatchRoleBindingForAllNamespacesListOptional<'_>,
    ) -> Result<(http::Request<Vec<u8>>, fn(http::StatusCode) -> crate::ResponseBody<WatchRoleBindingForAllNamespacesListResponse>), crate::RequestError> {
        let WatchRoleBindingForAllNamespacesListOptional {
            continue_,
            include_uninitialized,
            label_selector,
            limit,
...
/// Optional parameters of [`RoleBinding::watch_role_binding_for_all_namespaces_list`]
#[derive(Clone, Copy, Debug, Default)]
pub struct WatchRoleBindingForAllNamespacesListOptional<'a> {
    /// The continue option should be set when retrieving more results from the server. Since this value is server defined, clients may only use the continue value from a previous query result with identical query parameters (except for the value of continue) and the server may reject a continue value it does not recognize. If the specified continue value is no longer valid whether due to expiration (generally five to fifteen minutes) or a configuration change on the server the server will respond with a 410 ResourceExpired error indicating the client must restart their list without the continue field. This field is not supported when watch is true. Clients may start a watch from the last resourceVersion value returned by the server and not miss any modifications.
    pub continue_: Option<&'a str>,
    /// If true, partially initialized resources are included in the response.
    pub include_uninitialized: Option<bool>,
...
/// Use `<WatchRoleBindingForAllNamespacesListResponse as Response>::try_from_parts` to parse the HTTP response body of [`RoleBinding::watch_role_binding_for_all_namespaces_list`]
#[derive(Debug)]
pub enum WatchRoleBindingForAllNamespacesListResponse {
    Ok(crate::v1_8::apimachinery::pkg::apis::meta::v1::WatchEvent),
    Unauthorized,
    Other,
}

impl crate::Response for WatchRoleBindingForAllNamespacesListResponse {
    fn try_from_parts(status_code: http::StatusCode, buf: &[u8]) -> Result<(Self, usize), crate::ResponseError> {
        match status_code {
            http::StatusCode::OK => {
                let mut deserializer = serde_json::Deserializer::from_slice(buf).into_iter();
                ...
                Ok((WatchRoleBindingForAllNamespacesListResponse::Ok(result), byte_offset))
            },
            http::StatusCode::UNAUTHORIZED => Ok((WatchRoleBindingForAllNamespacesListResponse::Unauthorized, 0)),
            _ => Ok((WatchRoleBindingForAllNamespacesListResponse::Other, 0)),
        }
    }
}
...
impl RoleBinding {
    ...
    pub fn watch_role_binding_list_for_all_namespaces(
        ...
        let __url = "/apis/rbac.authorization.k8s.io/v1/watch/rolebindings?".to_string();
        let mut __query_pairs = url::form_urlencoded::Serializer::new(__url);
        if let Some(continue_) = continue_ {
            __query_pairs.append_pair("continue", continue_);
        }
