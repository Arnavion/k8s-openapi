...
impl NetworkPolicy {
    ...
    pub fn list_network_policy_for_all_namespaces(
        ...
        let __url = "/apis/networking.k8s.io/v1/networkpolicies?".to_string();
        let mut __query_pairs = url::form_urlencoded::Serializer::new(__url);
        if let Some(continue_) = continue_ {
            __query_pairs.append_pair("continue", continue_);
        }
...
impl crate::Response for WatchNamespacedNetworkPolicyListResponse {
    fn try_from_parts(status_code: http::StatusCode, buf: &[u8]) -> Result<(Self, usize), crate::ResponseError> {
        match status_code {
            http::StatusCode::OK => {
                let mut deserializer = serde_json::Deserializer::from_slice(buf).into_iter();
                ...
                Ok((WatchNamespacedNetworkPolicyListResponse::Ok(result), byte_offset))
            },
            http::StatusCode::UNAUTHORIZED => Ok((WatchNamespacedNetworkPolicyListResponse::Unauthorized, 0)),
            _ => Ok((WatchNamespacedNetworkPolicyListResponse::Other, 0)),
        }
    }
}
...
// Generated from operation watchNetworkingV1NetworkPolicyForAllNamespacesList
...
impl NetworkPolicy {
    ...
    /// This operation only supports watching a list of items for changes.
    ///
    /// Use the returned [`crate::ResponseBody`]`<`[`WatchNetworkPolicyForAllNamespacesListResponse`]`>` constructor, or [`WatchNetworkPolicyForAllNamespacesListResponse`] directly, to parse the HTTP response.
    ///
    /// # Arguments
    ///
    /// * `optional`
    ///
    ///     Optional parameters. Use `Default::default()` to not pass any.
    pub fn watch_network_policy_for_all_namespaces_list(
        optional: WatchNetworkPolicyForAllNamespacesListOptional<'_>,
    ) -> Result<(http::Request<Vec<u8>>, fn(http::StatusCode) -> crate::ResponseBody<WatchNetworkPolicyForAllNamespacesListResponse>), crate::RequestError> {
        let WatchNetworkPolicyForAllNamespacesListOptional {
            continue_,
            include_uninitialized,
            label_selector,
            limit,
...
/// Optional parameters of [`NetworkPolicy::watch_network_policy_for_all_namespaces_list`]
#[derive(Clone, Copy, Debug, Default)]
pub struct WatchNetworkPolicyForAllNamespacesListOptional<'a> {
    /// The continue option should be set when retrieving more results from the server. Since this value is server defined, clients may only use the continue value from a previous query result with identical query parameters (except for the value of continue) and the server may reject a continue value it does not recognize. If the specified continue value is no longer valid whether due to expiration (generally five to fifteen minutes) or a configuration change on the server the server will respond with a 410 ResourceExpired error indicating the client must restart their list without the continue field. This field is not supported when watch is true. Clients may start a watch from the last resourceVersion value returned by the server and not miss any modifications.
    pub continue_: Option<&'a str>,
    /// If true, partially initialized resources are included in the response.
    pub include_uninitialized: Option<bool>,
...
/// Use `<WatchNetworkPolicyForAllNamespacesListResponse as Response>::try_from_parts` to parse the HTTP response body of [`NetworkPolicy::watch_network_policy_for_all_namespaces_list`]
#[derive(Debug)]
pub enum WatchNetworkPolicyForAllNamespacesListResponse {
    Ok(crate::v1_11::apimachinery::pkg::apis::meta::v1::WatchEvent),
    Unauthorized,
    Other,
}

impl crate::Response for WatchNetworkPolicyForAllNamespacesListResponse {
    fn try_from_parts(status_code: http::StatusCode, buf: &[u8]) -> Result<(Self, usize), crate::ResponseError> {
        match status_code {
            http::StatusCode::OK => {
                let mut deserializer = serde_json::Deserializer::from_slice(buf).into_iter();
                ...
                Ok((WatchNetworkPolicyForAllNamespacesListResponse::Ok(result), byte_offset))
            },
            http::StatusCode::UNAUTHORIZED => Ok((WatchNetworkPolicyForAllNamespacesListResponse::Unauthorized, 0)),
            _ => Ok((WatchNetworkPolicyForAllNamespacesListResponse::Other, 0)),
        }
    }
}
...
impl NetworkPolicy {
    ...
    pub fn watch_network_policy_list_for_all_namespaces(
        ...
        let __url = "/apis/networking.k8s.io/v1/watch/networkpolicies?".to_string();
        let mut __query_pairs = url::form_urlencoded::Serializer::new(__url);
        if let Some(continue_) = continue_ {
            __query_pairs.append_pair("continue", continue_);
        }
...
impl crate::Response for WatchNetworkPolicyListForAllNamespacesResponse {
    fn try_from_parts(status_code: http::StatusCode, buf: &[u8]) -> Result<(Self, usize), crate::ResponseError> {
        match status_code {
            http::StatusCode::OK => {
                let mut deserializer = serde_json::Deserializer::from_slice(buf).into_iter();
                ...
                Ok((WatchNetworkPolicyListForAllNamespacesResponse::Ok(result), byte_offset))
            },
            http::StatusCode::UNAUTHORIZED => Ok((WatchNetworkPolicyListForAllNamespacesResponse::Unauthorized, 0)),
            _ => Ok((WatchNetworkPolicyListForAllNamespacesResponse::Other, 0)),
        }
    }
}
