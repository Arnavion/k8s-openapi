...
// Generated from operation watchCoreV1EndpointsForAllNamespacesList
...
impl Endpoints {
    ...
    ///
    /// Use the returned [`crate::ResponseBody`]`<`[`WatchEndpointsForAllNamespacesListResponse`]`>` constructor, or [`WatchEndpointsForAllNamespacesListResponse`] directly, to parse the HTTP response.
    ///
    /// # Arguments
    ///
    /// * `optional`
    ///
    ///     Optional parameters. Use `Default::default()` to not pass any.
    pub fn watch_endpoints_for_all_namespaces_list(
        optional: WatchEndpointsForAllNamespacesListOptional<'_>,
    ) -> Result<(http::Request<Vec<u8>>, fn(http::StatusCode) -> crate::ResponseBody<WatchEndpointsForAllNamespacesListResponse>), crate::RequestError> {
        let WatchEndpointsForAllNamespacesListOptional {
            continue_,
            include_uninitialized,
            label_selector,
            limit,
...
/// Optional parameters of [`Endpoints::watch_endpoints_for_all_namespaces_list`]
#[derive(Clone, Copy, Debug, Default)]
pub struct WatchEndpointsForAllNamespacesListOptional<'a> {
    /// The continue option should be set when retrieving more results from the server. Since this value is server defined, clients may only use the continue value from a previous query result with identical query parameters (except for the value of continue) and the server may reject a continue value it does not recognize. If the specified continue value is no longer valid whether due to expiration (generally five to fifteen minutes) or a configuration change on the server the server will respond with a 410 ResourceExpired error indicating the client must restart their list without the continue field. This field is not supported when watch is true. Clients may start a watch from the last resourceVersion value returned by the server and not miss any modifications.
    pub continue_: Option<&'a str>,
    /// If true, partially initialized resources are included in the response.
    pub include_uninitialized: Option<bool>,
...
/// Use `<WatchEndpointsForAllNamespacesListResponse as Response>::try_from_parts` to parse the HTTP response body of [`Endpoints::watch_endpoints_for_all_namespaces_list`]
#[derive(Debug)]
pub enum WatchEndpointsForAllNamespacesListResponse {
    Ok(crate::v1_11::apimachinery::pkg::apis::meta::v1::WatchEvent),
    Unauthorized,
    Other,
}

impl crate::Response for WatchEndpointsForAllNamespacesListResponse {
    fn try_from_parts(status_code: http::StatusCode, buf: &[u8]) -> Result<(Self, usize), crate::ResponseError> {
        match status_code {
            http::StatusCode::OK => {
                let mut deserializer = serde_json::Deserializer::from_slice(buf).into_iter();
                ...
                Ok((WatchEndpointsForAllNamespacesListResponse::Ok(result), byte_offset))
            },
            http::StatusCode::UNAUTHORIZED => Ok((WatchEndpointsForAllNamespacesListResponse::Unauthorized, 0)),
            _ => Ok((WatchEndpointsForAllNamespacesListResponse::Other, 0)),
        }
    }
}
...
impl Endpoints {
    ...
    pub fn watch_endpoints_list_for_all_namespaces(
        ...
        let __url = "/api/v1/watch/endpoints?".to_string();
        let mut __query_pairs = url::form_urlencoded::Serializer::new(__url);
        if let Some(continue_) = continue_ {
            __query_pairs.append_pair("continue", continue_);
        }
