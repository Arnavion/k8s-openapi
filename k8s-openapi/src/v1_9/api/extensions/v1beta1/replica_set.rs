...
impl ReplicaSet {
    ...
    pub fn list_replica_set_for_all_namespaces(
        ...
        let __url = "/apis/extensions/v1beta1/replicasets?".to_string();
        let mut __query_pairs = url::form_urlencoded::Serializer::new(__url);
        if let Some(continue_) = continue_ {
            __query_pairs.append_pair("continue", continue_);
        }
...
/// Optional parameters of [`ReplicaSet::watch_replica_set_for_all_namespaces`]
#[derive(Clone, Copy, Debug, Default)]
...
// Generated from operation watchExtensionsV1beta1ReplicaSetForAllNamespacesList
...
impl ReplicaSet {
    ...
    /// This operation only supports watching a list of items for changes.
    ///
    /// Use the returned [`crate::ResponseBody`]`<`[`WatchReplicaSetForAllNamespacesListResponse`]`>` constructor, or [`WatchReplicaSetForAllNamespacesListResponse`] directly, to parse the HTTP response.
    ///
    /// # Arguments
    ///
    /// * `optional`
    ///
    ///     Optional parameters. Use `Default::default()` to not pass any.
    pub fn watch_replica_set_for_all_namespaces_list(
        optional: WatchReplicaSetForAllNamespacesListOptional<'_>,
    ) -> Result<(http::Request<Vec<u8>>, fn(http::StatusCode) -> crate::ResponseBody<WatchReplicaSetForAllNamespacesListResponse>), crate::RequestError> {
        let WatchReplicaSetForAllNamespacesListOptional {
            continue_,
            include_uninitialized,
            label_selector,
            limit,
...
/// Optional parameters of [`ReplicaSet::watch_replica_set_for_all_namespaces_list`]
#[derive(Clone, Copy, Debug, Default)]
pub struct WatchReplicaSetForAllNamespacesListOptional<'a> {
    /// The continue option should be set when retrieving more results from the server. Since this value is server defined, clients may only use the continue value from a previous query result with identical query parameters (except for the value of continue) and the server may reject a continue value it does not recognize. If the specified continue value is no longer valid whether due to expiration (generally five to fifteen minutes) or a configuration change on the server the server will respond with a 410 ResourceExpired error indicating the client must restart their list without the continue field. This field is not supported when watch is true. Clients may start a watch from the last resourceVersion value returned by the server and not miss any modifications.
    pub continue_: Option<&'a str>,
    /// If true, partially initialized resources are included in the response.
    pub include_uninitialized: Option<bool>,
...
/// Use `<WatchReplicaSetForAllNamespacesListResponse as Response>::try_from_parts` to parse the HTTP response body of [`ReplicaSet::watch_replica_set_for_all_namespaces_list`]
#[derive(Debug)]
pub enum WatchReplicaSetForAllNamespacesListResponse {
    Ok(crate::v1_9::apimachinery::pkg::apis::meta::v1::WatchEvent),
    Unauthorized,
    Other,
}

impl crate::Response for WatchReplicaSetForAllNamespacesListResponse {
    fn try_from_parts(status_code: http::StatusCode, buf: &[u8]) -> Result<(Self, usize), crate::ResponseError> {
        match status_code {
            http::StatusCode::OK => {
                let mut deserializer = serde_json::Deserializer::from_slice(buf).into_iter();
                ...
                Ok((WatchReplicaSetForAllNamespacesListResponse::Ok(result), byte_offset))
            },
            http::StatusCode::UNAUTHORIZED => Ok((WatchReplicaSetForAllNamespacesListResponse::Unauthorized, 0)),
            _ => Ok((WatchReplicaSetForAllNamespacesListResponse::Other, 0)),
        }
    }
}
...
impl ReplicaSet {
    ...
    pub fn watch_replica_set_list_for_all_namespaces(
        ...
        let __url = "/apis/extensions/v1beta1/watch/replicasets?".to_string();
        let mut __query_pairs = url::form_urlencoded::Serializer::new(__url);
        if let Some(continue_) = continue_ {
            __query_pairs.append_pair("continue", continue_);
        }
