...
#[derive(Debug)]
pub enum GetAutoscalingV2alpha1APIResourcesResponse<R> where R: ::std::io::Read {
    Ok(::v1_7::apimachinery::pkg::apis::meta::v1::APIResourceList),
    Unauthorized(R),
    Other(::http::StatusCode, R),
}
...
/// get available resources
pub fn get_autoscaling_v2alpha1_api_resources<C>(
    __client: &C,
) -> Result<GetAutoscalingV2alpha1APIResourcesResponse<C::Response>, ::Error<C::Error>> where C: ::Client {
    let __url = __client.base_url().join(&format!("/apis/autoscaling/v2alpha1/")).map_err(::Error::URL)?;
    ...
    Ok(match ::Response::status_code(&response) {
        ::http::StatusCode::OK => {
            let result = ::serde_json::from_reader(response).map_err(::Error::JSON)?;
                GetAutoscalingV2alpha1APIResourcesResponse::Ok(result)
            },
        ::http::StatusCode::UNAUTHORIZED => GetAutoscalingV2alpha1APIResourcesResponse::Unauthorized(response),
        other => GetAutoscalingV2alpha1APIResourcesResponse::Other(other, response),
    })
}
