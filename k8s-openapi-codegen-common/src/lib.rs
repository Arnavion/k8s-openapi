...
pub fn run(
    ...
    let definition = definitions
        .get(definition_path)
        .ok_or_else(|| format!("definition for {definition_path} does not exist in spec"))?;
    ...
    let namespace_parts: Vec<_> = map_namespace
        .map_namespace(&path_parts)
        .ok_or_else(|| format!("unexpected path {definition_path:?}"))?
        .into_iter()
        .collect();

    let type_feature = if let swagger20::SchemaKind::Ty(
    ...
    let type_name = path_parts
        .last()
        .ok_or_else(|| format!("path for {definition_path} has no parts"))?;
    ...
    match &definition.kind {
        swagger20::SchemaKind::Properties(properties) => {
            let (template_properties, resource_metadata, metadata_ty) = {
                ...
                let mut single_group_version_kind =
                    match &definition.kubernetes_group_kind_versions[..] {
                ...
                for (name, (schema, required)) in properties {
                    ...
                    if let swagger20::SchemaKind::Ref(swagger20::RefPath { path, .. }) =
                        &schema.kind
                    {
                ...
                let resource_metadata = match single_group_version_kind {
                    ...
                    Some((_, true, false)) => {
                        return Err(format!(
                            "{definition_path} has an apiVersion property but not a kind property"
                        )
                        .into())
                    }
                    Some((_, false, true)) => {
                        return Err(format!(
                            "{definition_path} has a kind property but not an apiVersion property"
                        )
                        .into())
                    }
            ...
            if !definition.kubernetes_group_kind_versions.is_empty() {
                let mut kubernetes_group_kind_versions: Vec<_> =
                    definition.kubernetes_group_kind_versions.iter().collect();
                kubernetes_group_kind_versions.sort();

                let mut operations_by_gkv: std::collections::BTreeMap<_, Vec<_>> =
                    Default::default();
                ...
                for kubernetes_group_kind_version in kubernetes_group_kind_versions {
                    if let Some(mut operations) =
                        operations_by_gkv.remove(&Some(kubernetes_group_kind_version.clone()))
                    {
                        ...
                        writeln!(
                            out,
                            "// Begin {}/{}/{}",
                            kubernetes_group_kind_version.group,
                            kubernetes_group_kind_version.version,
                            kubernetes_group_kind_version.kind
                        )?;
                        ...
                        for operation in operations {
                            ...
                            state.handle_operation_types(
                                operation_optional_parameters_name.as_deref(),
                                operation_result_name.as_deref(),
                            )?;
                            ...
                            let components = (
                                components
                                    .next()
                                    .expect("str::rsplit returns at least one component"),
                                components.next(),
                                components.next(),
                                components.next(),
                            );

                            let (url_path_segment_, scope_, url_path_segment_and_scope) =
                                match components {
                                    (
                                        "{name}",
                                        Some(url_path_segment),
                                        Some("{namespace}"),
                                        Some("namespaces"),
                                    ) => (
            ...
            match &**definition_path {
                ...
                | "io.k8s.apimachinery.pkg.apis.meta.v1.APIVersions" => {
                    namespace_or_cluster_scoped_url_path_segment_and_scope
                        .push((r#""""#.to_owned(), format!("{local}ClusterResourceScope")))
                }
                "io.k8s.apimachinery.pkg.apis.meta.v1.Status" => {
                    subresource_url_path_segment_and_scope
                        .push((r#""status""#.to_owned(), format!("{local}SubResourceScope")))
                }
                _ => (),
            }
        ...
        swagger20::SchemaKind::Ty(swagger20::Type::IntOrString) => {
            templates::int_or_string::generate(&mut out, type_name, map_namespace)?;

            run_result.num_generated_structs += 1;
        }

        swagger20::SchemaKind::Ty(swagger20::Type::JsonSchemaPropsOr(
            namespace,
            ...
            let json_schema_props_or = match json_schema_props_or {
                ...
                swagger20::JsonSchemaPropsOr::StringArray => {
                    templates::json_schema_props_or::Or::StringArray
                }
            };
        ...
        swagger20::SchemaKind::Ty(swagger20::Type::Patch) => {
            templates::patch::generate(&mut out, type_name, map_namespace)?;
        ...
        swagger20::SchemaKind::Ty(swagger20::Type::ListDef { metadata }) => {
            ...
            {
                let template_generics_where_part =
                    format!("T: {local}serde::Deserialize<'de> + {local}ListableResource");
                let template_generics = templates::Generics {
                    where_part: Some(&template_generics_where_part),
                    ..template_generics
                };
            ...
            {
                let template_generics_where_part =
                    format!("T: {local}serde::Serialize + {local}ListableResource");
                let template_generics = templates::Generics {
                    where_part: Some(&template_generics_where_part),
                    ..template_generics
                };
        ...
        swagger20::SchemaKind::Ty(swagger20::Type::ListRef { .. }) => {
            return Err(format!("definition {definition_path} is a ListRef").into())
        }

        swagger20::SchemaKind::Ty(
            ty @ (swagger20::Type::CreateOptional(properties)
            ...
            match ty {
                ...
                swagger20::Type::DeleteOptional(_) => templates::impl_serialize::generate(
                    &mut out,
                    type_name,
                    template_generics,
                    &template_properties,
                ...
                swagger20::Type::WatchOptional(_) => templates::query_string_optional::generate(
                    &mut out,
                    type_name,
                    template_generics,
                    vis,
            ...
            let datetime_serialization_format = match (&**definition_path, &definition.kind) {
                (
                    ...
                    swagger20::SchemaKind::Ty(swagger20::Type::String {
                        format: Some(swagger20::StringFormat::DateTime),
                    }),
...
fn map_namespace_local_to_string(map_namespace: &impl MapNamespace) -> Result<String, Error> {
    let namespace_parts = map_namespace
        .map_namespace(&["io", "k8s"])
        .ok_or(r#"unexpected path "io.k8s""#)?;
...
fn get_derives(
    ...
    let derive_clone = evaluate_trait_bound(kind, true, definitions, map_namespace, |kind, _| {
        Ok(!matches!(
            kind,
            swagger20::SchemaKind::Ty(
                swagger20::Type::CreateResponse
    ...
    let derive_copy = derive_clone
        && evaluate_trait_bound(kind, false, definitions, map_namespace, |kind, _| {
            Ok(matches!(
                kind,
                swagger20::SchemaKind::Ty(
                    swagger20::Type::CreateOptional(_)
    ...
    let is_default = evaluate_trait_bound(
        kind,
        false,
        definitions,
        map_namespace,
        |kind, required| match kind {
            // Option<T>::default is None regardless of T
            _ if !required => Ok(true),

            swagger20::SchemaKind::Ref(swagger20::RefPath {
                can_be_default: Some(can_be_default),
                ..
            }) => Ok(*can_be_default),

            swagger20::SchemaKind::Ref(ref_path @ swagger20::RefPath { .. })
                if ref_path.references_scope(map_namespace) =>
            {
                Ok(false)
            }
            ...
            swagger20::SchemaKind::Ref(ref_path @ swagger20::RefPath { .. })
                if !ref_path.references_scope(map_namespace)
                    && ref_path.path == "io.k8s.apimachinery.pkg.apis.meta.v1.ObjectMeta" =>
            {
                Ok(true)
            }
    ...
    let derive_partial_eq =
        evaluate_trait_bound(kind, true, definitions, map_namespace, |kind, _| {
            Ok(!matches!(
                kind,
                swagger20::SchemaKind::Ty(
                    swagger20::Type::CreateResponse
    ...
    let derive_partial_ord = derive_partial_eq
        && matches!(
            kind,
            swagger20::SchemaKind::Ty(swagger20::Type::String {
                format: Some(swagger20::StringFormat::DateTime)
...
fn evaluate_trait_bound(
    ...
    fn evaluate_trait_bound_inner<'a>(
        ...
        // False positive. Clippy wants this to be `&SchemaKind` but we use Cow-specific operations (`.clone()`).
        ...
        match &**kind {
            swagger20::SchemaKind::Properties(properties) => {
                for (property_schema, property_required) in properties.values() {
                    ...
                    let field_bound = evaluate_trait_bound_inner(
            ...
            swagger20::SchemaKind::Ref(ref_path @ swagger20::RefPath { .. })
                if !ref_path.references_scope(map_namespace) =>
            {
                let trait_bound = if let Some(target) = definitions.get(&*ref_path.path) {
                    let mut visited = visited.clone();
            ...
            swagger20::SchemaKind::Ty(swagger20::Type::JsonSchemaPropsOr(namespace, _)) => {
                ...
                let json_schema_props_bound = evaluate_trait_bound_inner(
                    &std::borrow::Cow::Owned(swagger20::SchemaKind::Ref(
                        json_schema_props_ref_path,
                    )),
                    required,
                    array_follows_elements,
                    definitions,
            ...
            swagger20::SchemaKind::Ty(swagger20::Type::WatchEvent(raw_extension_ref_path)) => {
                let raw_extension_bound = evaluate_trait_bound_inner(
                    &std::borrow::Cow::Owned(swagger20::SchemaKind::Ref(
                        raw_extension_ref_path.clone(),
                    )),
                    required,
                    array_follows_elements,
                    definitions,
...
fn get_comment_text<'a>(
    s: &'a str,
    indent: &'a str,
) -> impl Iterator<Item = std::borrow::Cow<'static, str>> + 'a {
    s.lines().scan(true, move |previous_line_was_empty, line| {
        if line.is_empty() {
            *previous_line_was_empty = true;
            ...
            let line = if *previous_line_was_empty && line.starts_with("    ") {
...
pub fn get_rust_ident(name: &str) -> std::borrow::Cow<'static, str> {
    ...
    let chars = name
        .chars()
        .zip(std::iter::once(None).chain(name.chars().map(|c| Some(c))))
        .zip(
            name.chars()
                .skip(1)
                .map(|c| Some(c))
    ...
    for ((c, previous), next) in chars {
        if c.is_uppercase() {
            match (previous, next) {
                (Some(pc), _) if !pc.is_uppercase() => result.push('_'),
                (Some(pc), Some(nc)) if pc.is_uppercase() && !nc.is_uppercase() => result.push('_'),
                _ => (),
            }
            ...
            result.push(match c {
                '-' => '_',
                '.' if previous.is_some_and(|pc| pc.is_alphanumeric())
                    && next.is_some_and(|nc| nc.is_alphanumeric()) =>
                {
                    '_'
                }
                c => c,
...
fn get_rust_borrow_type(
    ...
    match schema_kind {
        ...
        swagger20::SchemaKind::Ref(swagger20::RefPath { path, .. })
            if path == "io.k8s.CreateOptional" =>
        {
            Ok(format!("{local}CreateOptional<'_>").into())
        }

        swagger20::SchemaKind::Ref(swagger20::RefPath { path, .. })
            if path == "io.k8s.DeleteOptional" =>
        {
            Ok(format!("{local}DeleteOptional<'_>").into())
        }

        swagger20::SchemaKind::Ref(swagger20::RefPath { path, .. })
            if path == "io.k8s.ListOptional" =>
        {
            Ok(format!("{local}ListOptional<'_>").into())
        }

        swagger20::SchemaKind::Ref(swagger20::RefPath { path, .. })
            if path == "io.k8s.PatchOptional" =>
        {
            Ok(format!("{local}PatchOptional<'_>").into())
        }

        swagger20::SchemaKind::Ref(swagger20::RefPath { path, .. })
            if path == "io.k8s.ReplaceOptional" =>
        {
            Ok(format!("{local}ReplaceOptional<'_>").into())
        }

        swagger20::SchemaKind::Ref(swagger20::RefPath { path, .. })
            if path == "io.k8s.WatchOptional" =>
        {
            Ok(format!("{local}WatchOptional<'_>").into())
        }

        swagger20::SchemaKind::Ref(ref_path) => Ok(format!(
            "&{}",
            get_fully_qualified_type_name(ref_path, map_namespace)
        )
        .into()),

        swagger20::SchemaKind::Ty(swagger20::Type::Any) => {
            Ok(format!("&{local}serde_json::Value").into())
        }
        ...
        swagger20::SchemaKind::Ty(swagger20::Type::Integer {
            format: swagger20::IntegerFormat::Int32,
        }) => Ok("i32".into()),
        swagger20::SchemaKind::Ty(swagger20::Type::Integer {
            format: swagger20::IntegerFormat::Int64,
        }) => Ok("i64".into()),

        swagger20::SchemaKind::Ty(swagger20::Type::Number {
            format: swagger20::NumberFormat::Double,
        }) => Ok("f64".into()),
        ...
        swagger20::SchemaKind::Ty(swagger20::Type::Object {
            ...
            get_rust_type(&additional_properties.kind, map_namespace)?
        )
        .into()),

        swagger20::SchemaKind::Ty(swagger20::Type::String {
            format: Some(swagger20::StringFormat::Byte),
        ...
        swagger20::SchemaKind::Ty(swagger20::Type::IntOrString) => {
            Err("nothing should be trying to refer to IntOrString".into())
        }

        swagger20::SchemaKind::Ty(swagger20::Type::JsonSchemaPropsOr(_, _)) => {
            Err("JSON schema types not supported".into())
        }
        swagger20::SchemaKind::Ty(swagger20::Type::Patch) => Err("Patch type not supported".into()),
        swagger20::SchemaKind::Ty(swagger20::Type::WatchEvent(_)) => {
            Err("WatchEvent type not supported".into())
        }

        swagger20::SchemaKind::Ty(swagger20::Type::ListDef { .. }) => {
            Err("ListDef type not supported".into())
        }
        swagger20::SchemaKind::Ty(swagger20::Type::ListRef { .. }) => {
            Ok(format!("&{}", get_rust_type(schema_kind, map_namespace)?).into())
        }
        ...
        swagger20::SchemaKind::Ty(swagger20::Type::CreateOptional(_)) => {
            Err("CreateOptional type not supported".into())
        }
        swagger20::SchemaKind::Ty(swagger20::Type::DeleteOptional(_)) => {
            Err("DeleteOptional type not supported".into())
        }
        swagger20::SchemaKind::Ty(swagger20::Type::ListOptional(_)) => {
            Err("ListOptional type not supported".into())
        }
        swagger20::SchemaKind::Ty(swagger20::Type::PatchOptional(_)) => {
            Err("PatchOptional type not supported".into())
        }
        swagger20::SchemaKind::Ty(swagger20::Type::ReplaceOptional(_)) => {
            Err("ReplaceOptional type not supported".into())
        }
        swagger20::SchemaKind::Ty(swagger20::Type::WatchOptional(_)) => {
            Err("WatchOptional type not supported".into())
        }

        swagger20::SchemaKind::Ty(swagger20::Type::CreateResponse) => {
            Err("CreateResponse type not supported".into())
        }
        swagger20::SchemaKind::Ty(swagger20::Type::DeleteResponse) => {
            Err("DeleteResponse type not supported".into())
        }
        swagger20::SchemaKind::Ty(swagger20::Type::ListResponse) => {
            Err("ListResponse type not supported".into())
        }
        swagger20::SchemaKind::Ty(swagger20::Type::PatchResponse) => {
            Err("PatchResponse type not supported".into())
        }
        swagger20::SchemaKind::Ty(swagger20::Type::ReplaceResponse) => {
            Err("ReplaceResponse type not supported".into())
        }
        swagger20::SchemaKind::Ty(swagger20::Type::WatchResponse) => {
            Err("WatchResponse type not supported".into())
        }
    }
}
...
pub fn write_operation(
    ...
    let parameters: Result<Vec<_>, Error> = operation
        .parameters
        .iter()
    ...
    let delete_optional_parameter = parameters
        .iter()
    ...
    let query_string_optional_parameter = parameters
        .iter()
        .position(|(_, _, parameter)| {
            if let swagger20::SchemaKind::Ref(swagger20::RefPath { path, .. }) =
                &parameter.schema.kind
    ...
    parameters.sort_by(|(_, _, parameter1), (_, _, parameter2)| {
        (match (parameter1.location, parameter2.location) {
            ...
            | (swagger20::ParameterLocation::Body, swagger20::ParameterLocation::Query) => {
    ...
    let (required_parameters, optional_parameters): (Vec<_>, Vec<_>) = parameters
        .iter()
        .partition(|(_, _, parameter)| parameter.required);
    let any_optional_fields_have_lifetimes = optional_parameters
        .iter()
        .any(|(_, parameter_type, _)| parameter_type.starts_with('&'));
    ...
    if !parameters.is_empty()
        || delete_optional_parameter.is_some()
    ...
    if !optional_parameters.is_empty() {
        write!(
            out,
            "{indent}    optional: {operation_optional_parameters_name}"
        )?;
    ...
    if let Some(operation_result_name) = &operation_result_name {
        ...
        if let Some(common_response_type) = common_response_type {
            ...
            writeln!(
                out,
                "{indent}) -> Result<{local}http::Request<Vec<u8>>, {local}RequestError> {{"
            )?;
        }
    }

    let have_path_parameters = parameters
        .iter()
        .any(|(_, _, parameter)| parameter.location == swagger20::ParameterLocation::Path);
    let have_query_parameters = parameters
        .iter()
        .any(|(_, _, parameter)| parameter.location == swagger20::ParameterLocation::Query)
        || query_string_optional_parameter.is_some();

    if !optional_parameters.is_empty() {
        writeln!(
            out,
            "{indent}    let {operation_optional_parameters_name} {{"
        )?;
            ...
            for (parameter_name, parameter_type, parameter) in &parameters {
                if parameter.location == swagger20::ParameterLocation::Path {
                    match parameter.schema.kind {
                        ...
                        _ => {
                            return Err(format!(
                            "parameter {parameter_name} is in the path but is a {parameter_type:?}"
                        )
                            .into())
                        }
                    }
            ...
            for (parameter_name, parameter_type, parameter) in &parameters {
                if parameter.location == swagger20::ParameterLocation::Query {
                    if parameter.required {
                        ...
                        writeln!(
                            out,
                            "{indent}    if let Some({parameter_name}) = {parameter_name} {{"
                        )?;
                        match &parameter.schema.kind {
							swagger20::SchemaKind::Ty(swagger20::Type::Array { items }) => match &items.kind {
								swagger20::SchemaKind::Ty(swagger20::Type::String { .. }) => {
    ...
    writeln!(
        out,
        "{indent}    let __request = {local}http::Request::{method}(__url);"
    )?;

    write!(out, "{indent}    let __body = ")?;
    let body_parameter = if let Some((parameter_name, _, parameter)) = &delete_optional_parameter {
    ...
    } else if let Some((parameter_name, parameter_type, parameter)) = parameters
        .iter()
    ...
    if let Some((parameter_name, parameter)) = body_parameter {
        let is_patch = if let swagger20::SchemaKind::Ref(ref_path) = &parameter.schema.kind {
        ...
        if is_patch {
            ...
            writeln!(
                out,
                r#"{indent}        {patch_type}::Json(_) => "application/json-patch+json","#
            )?;
            writeln!(
                out,
                r#"{indent}        {patch_type}::Merge(_) => "application/merge-patch+json","#
            )?;
            writeln!(
                out,
                r#"{indent}        {patch_type}::StrategicMerge(_) => "application/strategic-merge-patch+json","#
            )?;
            writeln!(out, "{indent}    }}));")?;
        } else {
            writeln!(
    ...
    if operation_result_name.is_some() {
        ...
        writeln!(
            out,
            "{indent}        Ok(request) => Ok((request, {local}ResponseBody::new)),"
        )?;
        writeln!(
            out,
            "{indent}        Err(err) => Err({local}RequestError::Http(err)),"
        )?;
        writeln!(out, "{indent}    }}")?;
    } else {
        ...
        if is_common_response_type {
            ...
            writeln!(
                out,
                "{indent}    __request.body(__body).map_err({local}RequestError::Http)"
            )?;
        }
    }
    writeln!(out, "{indent}}}")?;
    ...
    if !optional_parameters.is_empty() {
        ...
        if let Some(type_name) = type_name {
            writeln!(
                out,
                "/// Optional parameters of [`{type_name}::{operation_fn_name}`]"
            )?;
        } else {
        ...
        for (parameter_name, parameter_type, parameter) in &optional_parameters {
            ...
            if let Some(borrowed_parameter_type) = parameter_type.strip_prefix('&') {
                writeln!(
                    out,
                    "    {vis}{parameter_name}: Option<&'a {borrowed_parameter_type}>,"
                )?;
            } else {
    ...
    if let swagger20::OperationResponses::Map(responses) = &operation.responses {
        if let Some(operation_result_name) = &operation_result_name {
            ...
            let operation_responses: Result<Vec<_>, _> = responses
                .iter()
            ...
            for &(_, variant_name, schema) in &operation_responses {
                writeln!(
                    out,
                    "    {variant_name}({}),",
                    get_rust_type(&schema.kind, map_namespace)?
                )?;
            }

            writeln!(
                out,
                "    Other(Result<Option<{local}serde_json::Value>, {local}serde_json::Error>),"
            )?;
            writeln!(out, "}}")?;
            writeln!(out)?;
            ...
            for &(http_status_code, variant_name, schema) in &operation_responses {
                writeln!(
                    out,
                    "            {local}http::StatusCode::{http_status_code} => {{"
                )?;
            ...
            writeln!(
                out,
                "                Ok(({operation_result_name}::Other(result), read))"
            )?;
            writeln!(out, "            }},")?;
            writeln!(out, "        }}")?;
            writeln!(out, "    }}")?;
...
fn get_operation_names(
    ...
    let (operation_id, operation_id_with_type_name) = if let Some(type_name) = type_name {
        ...
        if let Some(operation_id) = operation.id.strip_suffix(type_name) {
            ...
            (
                std::borrow::Cow::Owned(operation_id),
                std::borrow::Cow::Owned(operation_id_with_type_name),
            )
        } else {
    ...
    let first_char = chars
        .next()
        .ok_or_else(|| format!("operation has empty ID: {operation:?}"))?
        .to_uppercase();
    let rest_chars = chars.as_str();
