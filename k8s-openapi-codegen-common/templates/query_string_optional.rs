...
{operation_feature_attribute}impl{type_generics_impl} {type_name}{type_generics_type}{type_generics_where} {{
    #[doc(hidden)]
    /// Serializes this object to a [`{local}url::form_urlencoded::Serializer`]
    ///
    ...
    {vis}fn __serialize<T>(
        self,
        __query_pairs: &mut {local}url::form_urlencoded::Serializer<'_, T>,
    ) where T: {local}url::form_urlencoded::Target {{
