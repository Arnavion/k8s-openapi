...
impl ServiceSpec  {
    ...
    /// Set [`Self::ip_family`]
    pub  fn ip_family_set(&mut self, ip_family: impl Into<Option<String>>) -> &mut Self {
        self.ip_family = ip_family.into(); self
    }

    pub  fn ip_family(&mut self) -> &mut String {
        if self.ip_family.is_none() { self.ip_family = Some(Default::default()) }
        self.ip_family.as_mut().unwrap()
    }

    /// Modify [`Self::ip_family`] with a `func`
    ///
    /// The field will be set to `Default::default()` if not set before
    pub  fn ip_family_with(&mut self, func: impl FnOnce(&mut String)) -> &mut Self {
        if self.ip_family.is_none() { self.ip_family = Some(Default::default()) };
        func(self.ip_family.as_mut().unwrap()); self
    }
