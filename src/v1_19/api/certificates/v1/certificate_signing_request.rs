...
impl CertificateSigningRequest  {
    ...
    /// Set [`Self::spec`]
    pub  fn spec_set(&mut self, spec: impl Into<crate::api::certificates::v1::CertificateSigningRequestSpec>) -> &mut Self {
        self.spec = spec.into(); self
    }

    pub  fn spec(&mut self) -> &mut crate::api::certificates::v1::CertificateSigningRequestSpec {
        &mut self.spec
    }

    /// Modify [`Self::spec`] with a `func`
    pub  fn spec_with(&mut self, func: impl FnOnce(&mut crate::api::certificates::v1::CertificateSigningRequestSpec)) -> &mut Self {
        func(&mut self.spec); self
    }


    /// Set [`Self::status`]
    pub  fn status_set(&mut self, status: impl Into<Option<crate::api::certificates::v1::CertificateSigningRequestStatus>>) -> &mut Self {
        self.status = status.into(); self
    }

    pub  fn status(&mut self) -> &mut crate::api::certificates::v1::CertificateSigningRequestStatus {
    ...
    pub  fn status_with(&mut self, func: impl FnOnce(&mut crate::api::certificates::v1::CertificateSigningRequestStatus)) -> &mut Self {
        if self.status.is_none() { self.status = Some(Default::default()) };
