...
impl ValidatingWebhook  {
    ...
    pub  fn admission_review_versions_set(&mut self, admission_review_versions: impl Into<Option<Vec<String>>>) -> &mut Self {
    ...
    pub  fn admission_review_versions(&mut self) -> &mut Vec<String> {
        if self.admission_review_versions.is_none() { self.admission_review_versions = Some(Default::default()) }
        self.admission_review_versions.as_mut().unwrap()
    }
    ...
    pub  fn admission_review_versions_with(&mut self, func: impl FnOnce(&mut Vec<String>)) -> &mut Self {
        if self.admission_review_versions.is_none() { self.admission_review_versions = Some(Default::default()) };
        func(self.admission_review_versions.as_mut().unwrap()); self
    }
    ...
    pub  fn admission_review_versions_push_with(&mut self, func: impl FnOnce(&mut String)) -> &mut Self {
        if self.admission_review_versions.is_none() {
            self.admission_review_versions = Some(vec![]);
        }
        let mut new = Default::default();
        func(&mut new);
        self.admission_review_versions.as_mut().unwrap().push(new);
        self
    }
    ...
    pub  fn admission_review_versions_append_from(&mut self, other: impl std::borrow::Borrow<[String]>) -> &mut Self {
         if self.admission_review_versions.is_none() { self.admission_review_versions = Some(Vec::new()); }
         let admission_review_versions = &mut self.admission_review_versions.as_mut().unwrap();
         for item in other.borrow() {
             admission_review_versions.push(item.to_owned());
         }
         self
    }
    ...
    pub  fn client_config_set(&mut self, client_config: impl Into<crate::api::admissionregistration::v1beta1::WebhookClientConfig>) -> &mut Self {
        self.client_config = client_config.into(); self
    }

    pub  fn client_config(&mut self) -> &mut crate::api::admissionregistration::v1beta1::WebhookClientConfig {
        &mut self.client_config
    }

    /// Modify [`Self::client_config`] with a `func`
    pub  fn client_config_with(&mut self, func: impl FnOnce(&mut crate::api::admissionregistration::v1beta1::WebhookClientConfig)) -> &mut Self {
    ...
    /// Set [`Self::rules`]
    pub  fn rules_set(&mut self, rules: impl Into<Option<Vec<crate::api::admissionregistration::v1beta1::RuleWithOperations>>>) -> &mut Self {
        self.rules = rules.into(); self
    }

    pub  fn rules(&mut self) -> &mut Vec<crate::api::admissionregistration::v1beta1::RuleWithOperations> {
    ...
    ///
    /// The field will be set to `Default::default()` if not set before
    pub  fn rules_with(&mut self, func: impl FnOnce(&mut Vec<crate::api::admissionregistration::v1beta1::RuleWithOperations>)) -> &mut Self {
        if self.rules.is_none() { self.rules = Some(Default::default()) };
    ...
    ///
    /// The field will initially set to `Default::default()`
    pub  fn rules_push_with(&mut self, func: impl FnOnce(&mut crate::api::admissionregistration::v1beta1::RuleWithOperations)) -> &mut Self {
    ...
    pub  fn rules_append_from(&mut self, other: impl std::borrow::Borrow<[crate::api::admissionregistration::v1beta1::RuleWithOperations]>) -> &mut Self {
    ...
    pub  fn side_effects_set(&mut self, side_effects: impl Into<Option<String>>) -> &mut Self {
        self.side_effects = side_effects.into(); self
    }

    pub  fn side_effects(&mut self) -> &mut String {
        if self.side_effects.is_none() { self.side_effects = Some(Default::default()) }
        self.side_effects.as_mut().unwrap()
    }
    ...
    pub  fn side_effects_with(&mut self, func: impl FnOnce(&mut String)) -> &mut Self {
        if self.side_effects.is_none() { self.side_effects = Some(Default::default()) };
        func(self.side_effects.as_mut().unwrap()); self
    }
