...
impl crate::Schema for VolumeAttachmentSource {
    fn schema() -> serde_json::Value {
        serde_json::json!({
          ...
          "properties": {
            "inlineVolumeSpec": crate::schema_ref_with_values(crate::api::core::v1::PersistentVolumeSpec::schema(), serde_json::json!({"description": "inlineVolumeSpec contains all the information necessary to attach a persistent volume defined by a pod's inline VolumeSource. This field is populated only for the CSIMigration feature. It contains translated fields from a pod's inline VolumeSource to a PersistentVolumeSpec. This field is beta-level and is only honored by servers that enabled the CSIMigration feature."})),
            "persistentVolumeName": {
