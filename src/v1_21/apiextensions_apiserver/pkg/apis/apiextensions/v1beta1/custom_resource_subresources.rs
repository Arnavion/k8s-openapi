...
impl crate::Schema for CustomResourceSubresources {
    fn schema() -> serde_json::Value {
        serde_json::json!({
          ...
          "properties": {
            "scale": crate::schema_ref_with_values(crate::apiextensions_apiserver::pkg::apis::apiextensions::v1beta1::CustomResourceSubresourceScale::schema(), serde_json::json!({"description": "scale indicates the custom resource should serve a `/scale` subresource that returns an `autoscaling/v1` Scale object."})),
            "status": crate::schema_ref_with_values(crate::apiextensions_apiserver::pkg::apis::apiextensions::v1beta1::CustomResourceSubresourceStatus::schema(), serde_json::json!({"description": "status indicates the custom resource should serve a `/status` subresource. When enabled: 1. requests to the custom resource primary endpoint ignore changes to the `status` stanza of the object. 2. requests to the custom resource `/status` subresource ignore changes to anything other than the `status` stanza of the object."}))
          },
          "type": "object"
        })
    }
