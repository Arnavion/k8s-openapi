...
impl FlowSchemaSpec {
    pub fn schema() -> serde_json::Value {
        serde_json::json!({
          ...
          "properties": {
            "distinguisherMethod": crate::schema_ref_with_description(crate::api::flowcontrol::v1alpha1::FlowDistinguisherMethod::schema(), "`distinguisherMethod` defines how to compute the flow distinguisher for requests that match this schema. `nil` specifies that the distinguisher is disabled and thus will always be the empty string."),
            "matchingPrecedence": {
              "description": "`matchingPrecedence` is used to choose among the FlowSchemas that match a given request. The chosen FlowSchema is among those with the numerically lowest (which we take to be logically highest) MatchingPrecedence.  Each MatchingPrecedence value must be non-negative. Note that if the precedence is not specified or zero, it will be set to 1000 as default.",
              "format": "int32",
              "type": "integer"
            },
            "priorityLevelConfiguration": crate::schema_ref_with_description(crate::api::flowcontrol::v1alpha1::PriorityLevelConfigurationReference::schema(), "`priorityLevelConfiguration` should reference a PriorityLevelConfiguration in the cluster. If the reference cannot be resolved, the FlowSchema will be ignored and marked as invalid in its status. Required."),
            "rules": {
