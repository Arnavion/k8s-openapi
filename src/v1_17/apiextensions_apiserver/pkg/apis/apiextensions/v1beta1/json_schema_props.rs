        ...
        impl<'de> serde::de::Visitor<'de> for Visitor {
            ...
            fn visit_seq<A>(self, mut seq: A) -> Result<Self::Value, A::Error> where A: serde::de::SeqAccess<'de> {
                Ok(JSONSchemaProps {
                    ...
                    x_kubernetes_list_map_keys: serde::de::SeqAccess::next_element(&mut seq)?.ok_or_else(|| serde::de::Error::missing_field("x_kubernetes_list_map_keys"))?,
                    x_kubernetes_list_type: serde::de::SeqAccess::next_element(&mut seq)?.ok_or_else(|| serde::de::Error::missing_field("x_kubernetes_list_type"))?,
                    x_kubernetes_map_type: serde::de::SeqAccess::next_element(&mut seq)?.ok_or_else(|| serde::de::Error::missing_field("x_kubernetes_map_type"))?,
...
impl serde::Serialize for JSONSchemaProps {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error> where S: serde::Serializer {
        ...
        if let Some(value) = &self.x_kubernetes_list_map_keys {
            serde::ser::SerializeStruct::serialize_field(&mut state, "x-kubernetes-list-map-keys", &Some(value))?;
        }
        else {
            serde::ser::SerializeStruct::skip_field(&mut state, "x-kubernetes-list-map-keys")?;
        }
        if let Some(value) = &self.x_kubernetes_list_type {
            serde::ser::SerializeStruct::serialize_field(&mut state, "x-kubernetes-list-type", &Some(value))?;
        }
        else {
            serde::ser::SerializeStruct::skip_field(&mut state, "x-kubernetes-list-type")?;
        }
        if let Some(value) = &self.x_kubernetes_map_type {
            serde::ser::SerializeStruct::serialize_field(&mut state, "x-kubernetes-map-type", &Some(value))?;
        }
        else {
            serde::ser::SerializeStruct::skip_field(&mut state, "x-kubernetes-map-type")?;
        }
