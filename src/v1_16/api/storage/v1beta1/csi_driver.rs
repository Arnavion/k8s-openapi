    ...
    fn schema() -> serde_json::Value {
        serde_json::json!({
          ...
          "properties": {
            ...
            "metadata": crate::schema_ref_with_values(crate::apimachinery::pkg::apis::meta::v1::ObjectMeta::schema(), serde_json::json!({"description": "Standard object metadata. metadata.Name indicates the name of the CSI driver that this object refers to; it MUST be the same name returned by the CSI GetPluginName() call for that driver. The driver name must be 63 characters or less, beginning and ending with an alphanumeric character ([a-z0-9A-Z]) with dashes (-), dots (.), and alphanumerics between. More info: https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata"})),
            "spec": crate::schema_ref_with_values(crate::api::storage::v1beta1::CSIDriverSpec::schema(), serde_json::json!({"description": "Specification of the CSI Driver."}))
          },
          "required": [
            "metadata",
            "spec"
