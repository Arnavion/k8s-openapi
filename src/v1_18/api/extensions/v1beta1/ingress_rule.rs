...
impl IngressRule  {
    ...
    /// Set [`Self::http`]
    pub  fn http_set(&mut self, http: impl Into<Option<crate::api::extensions::v1beta1::HTTPIngressRuleValue>>) -> &mut Self {
        self.http = http.into(); self
    }

    pub  fn http(&mut self) -> &mut crate::api::extensions::v1beta1::HTTPIngressRuleValue {
    ...
    ///
    /// The field will be set to `Default::default()` if not set before
    pub  fn http_with(&mut self, func: impl FnOnce(&mut crate::api::extensions::v1beta1::HTTPIngressRuleValue)) -> &mut Self {
        if self.http.is_none() { self.http = Some(Default::default()) };
