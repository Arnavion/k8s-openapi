...
impl CertificateSigningRequestSpec  {
    ...
    pub  fn signer_name_set(&mut self, signer_name: impl Into<Option<String>>) -> &mut Self {
        self.signer_name = signer_name.into(); self
    }

    pub  fn signer_name(&mut self) -> &mut String {
        if self.signer_name.is_none() { self.signer_name = Some(Default::default()) }
        self.signer_name.as_mut().unwrap()
    }
    ...
    pub  fn signer_name_with(&mut self, func: impl FnOnce(&mut String)) -> &mut Self {
        if self.signer_name.is_none() { self.signer_name = Some(Default::default()) };
        func(self.signer_name.as_mut().unwrap()); self
    }
