...
impl VolumeAttachment  {
    ...
    /// Set [`Self::spec`]
    pub  fn spec_set(&mut self, spec: impl Into<crate::api::storage::v1::VolumeAttachmentSpec>) -> &mut Self {
        self.spec = spec.into(); self
    }

    pub  fn spec(&mut self) -> &mut crate::api::storage::v1::VolumeAttachmentSpec {
        &mut self.spec
    }

    /// Modify [`Self::spec`] with a `func`
    pub  fn spec_with(&mut self, func: impl FnOnce(&mut crate::api::storage::v1::VolumeAttachmentSpec)) -> &mut Self {
        func(&mut self.spec); self
    }


    /// Set [`Self::status`]
    pub  fn status_set(&mut self, status: impl Into<Option<crate::api::storage::v1::VolumeAttachmentStatus>>) -> &mut Self {
        self.status = status.into(); self
    }

    pub  fn status(&mut self) -> &mut crate::api::storage::v1::VolumeAttachmentStatus {
    ...
    pub  fn status_with(&mut self, func: impl FnOnce(&mut crate::api::storage::v1::VolumeAttachmentStatus)) -> &mut Self {
        if self.status.is_none() { self.status = Some(Default::default()) };
