...
impl MetricSpec {
    pub fn schema() -> serde_json::Value {
        serde_json::json!({
          ...
          "properties": {
            "containerResource": crate::schema_ref_with_description(crate::api::autoscaling::v2beta1::ContainerResourceMetricSource::schema(), "container resource refers to a resource metric (such as those specified in requests and limits) known to Kubernetes describing a single container in each pod of the current scale target (e.g. CPU or memory). Such metrics are built in to Kubernetes, and have special scaling options on top of those available to normal per-pod metrics using the \"pods\" source. This is an alpha feature and can be enabled by the HPAContainerMetrics feature flag."),
            "external": crate::schema_ref_with_description(crate::api::autoscaling::v2beta1::ExternalMetricSource::schema(), "external refers to a global metric that is not associated with any Kubernetes object. It allows autoscaling based on information coming from components running outside of cluster (for example length of queue in cloud messaging service, or QPS from loadbalancer running outside of cluster)."),
            "object": crate::schema_ref_with_description(crate::api::autoscaling::v2beta1::ObjectMetricSource::schema(), "object refers to a metric describing a single kubernetes object (for example, hits-per-second on an Ingress object)."),
            "pods": crate::schema_ref_with_description(crate::api::autoscaling::v2beta1::PodsMetricSource::schema(), "pods refers to a metric describing each pod in the current scale target (for example, transactions-processed-per-second).  The values will be averaged together before being compared to the target value."),
            "resource": crate::schema_ref_with_description(crate::api::autoscaling::v2beta1::ResourceMetricSource::schema(), "resource refers to a resource metric (such as those specified in requests and limits) known to Kubernetes describing each pod in the current scale target (e.g. CPU or memory). Such metrics are built in to Kubernetes, and have special scaling options on top of those available to normal per-pod metrics using the \"pods\" source."),
            "type": {
