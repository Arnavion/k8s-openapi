...
impl PodSpec  {
    ...
    /// Set [`Self::os`]
    pub  fn os_set(&mut self, os: impl Into<Option<crate::api::core::v1::PodOS>>) -> &mut Self {
        self.os = os.into(); self
    }

    pub  fn os(&mut self) -> &mut crate::api::core::v1::PodOS {
        if self.os.is_none() { self.os = Some(Default::default()) }
        self.os.as_mut().unwrap()
    }

    /// Modify [`Self::os`] with a `func`
    ///
    /// The field will be set to `Default::default()` if not set before
    pub  fn os_with(&mut self, func: impl FnOnce(&mut crate::api::core::v1::PodOS)) -> &mut Self {
        if self.os.is_none() { self.os = Some(Default::default()) };
        func(self.os.as_mut().unwrap()); self
    }
    ...
    /// Set [`Self::set_hostname_as_fqdn`]
    pub  fn set_hostname_as_fqdn_set(&mut self, set_hostname_as_fqdn: impl Into<Option<bool>>) -> &mut Self {
        self.set_hostname_as_fqdn = set_hostname_as_fqdn.into(); self
    }

    pub  fn set_hostname_as_fqdn(&mut self) -> &mut bool {
        if self.set_hostname_as_fqdn.is_none() { self.set_hostname_as_fqdn = Some(Default::default()) }
        self.set_hostname_as_fqdn.as_mut().unwrap()
    }

    /// Modify [`Self::set_hostname_as_fqdn`] with a `func`
    ///
    /// The field will be set to `Default::default()` if not set before
    pub  fn set_hostname_as_fqdn_with(&mut self, func: impl FnOnce(&mut bool)) -> &mut Self {
        if self.set_hostname_as_fqdn.is_none() { self.set_hostname_as_fqdn = Some(Default::default()) };
        func(self.set_hostname_as_fqdn.as_mut().unwrap()); self
    }
