...
impl HTTPIngressPath  {
    /// Set [`Self::backend`]
    pub  fn backend_set(&mut self, backend: impl Into<crate::api::networking::v1::IngressBackend>) -> &mut Self {
        self.backend = backend.into(); self
    }

    pub  fn backend(&mut self) -> &mut crate::api::networking::v1::IngressBackend {
        &mut self.backend
    }

    /// Modify [`Self::backend`] with a `func`
    pub  fn backend_with(&mut self, func: impl FnOnce(&mut crate::api::networking::v1::IngressBackend)) -> &mut Self {
    ...
    /// Set [`Self::path_type`]
    pub  fn path_type_set(&mut self, path_type: impl Into<String>) -> &mut Self {
