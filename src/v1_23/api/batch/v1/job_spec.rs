...
impl JobSpec  {
    ...
    /// Set [`Self::completion_mode`]
    pub  fn completion_mode_set(&mut self, completion_mode: impl Into<Option<String>>) -> &mut Self {
        self.completion_mode = completion_mode.into(); self
    }

    pub  fn completion_mode(&mut self) -> &mut String {
        if self.completion_mode.is_none() { self.completion_mode = Some(Default::default()) }
        self.completion_mode.as_mut().unwrap()
    }

    /// Modify [`Self::completion_mode`] with a `func`
    ///
    /// The field will be set to `Default::default()` if not set before
    pub  fn completion_mode_with(&mut self, func: impl FnOnce(&mut String)) -> &mut Self {
        if self.completion_mode.is_none() { self.completion_mode = Some(Default::default()) };
        func(self.completion_mode.as_mut().unwrap()); self
    }
