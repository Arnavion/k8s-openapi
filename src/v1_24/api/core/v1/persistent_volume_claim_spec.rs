...
impl PersistentVolumeClaimSpec  {
    ...
    /// Set [`Self::data_source_ref`]
    pub  fn data_source_ref_set(&mut self, data_source_ref: impl Into<Option<crate::api::core::v1::TypedLocalObjectReference>>) -> &mut Self {
        self.data_source_ref = data_source_ref.into(); self
    }

    pub  fn data_source_ref(&mut self) -> &mut crate::api::core::v1::TypedLocalObjectReference {
        if self.data_source_ref.is_none() { self.data_source_ref = Some(Default::default()) }
        self.data_source_ref.as_mut().unwrap()
    }

    /// Modify [`Self::data_source_ref`] with a `func`
    ///
    /// The field will be set to `Default::default()` if not set before
    pub  fn data_source_ref_with(&mut self, func: impl FnOnce(&mut crate::api::core::v1::TypedLocalObjectReference)) -> &mut Self {
        if self.data_source_ref.is_none() { self.data_source_ref = Some(Default::default()) };
        func(self.data_source_ref.as_mut().unwrap()); self
    }
