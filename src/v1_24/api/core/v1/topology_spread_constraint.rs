...
impl TopologySpreadConstraint  {
    ...
    /// Set [`Self::min_domains`]
    pub  fn min_domains_set(&mut self, min_domains: impl Into<Option<i32>>) -> &mut Self {
        self.min_domains = min_domains.into(); self
    }

    pub  fn min_domains(&mut self) -> &mut i32 {
        if self.min_domains.is_none() { self.min_domains = Some(Default::default()) }
        self.min_domains.as_mut().unwrap()
    }

    /// Modify [`Self::min_domains`] with a `func`
    ///
    /// The field will be set to `Default::default()` if not set before
    pub  fn min_domains_with(&mut self, func: impl FnOnce(&mut i32)) -> &mut Self {
        if self.min_domains.is_none() { self.min_domains = Some(Default::default()) };
        func(self.min_domains.as_mut().unwrap()); self
    }
