...
impl NetworkPolicy  {
    ...
    /// Set [`Self::status`]
    pub  fn status_set(&mut self, status: impl Into<Option<crate::api::networking::v1::NetworkPolicyStatus>>) -> &mut Self {
        self.status = status.into(); self
    }

    pub  fn status(&mut self) -> &mut crate::api::networking::v1::NetworkPolicyStatus {
    ...
    ///
    /// The field will be set to `Default::default()` if not set before
    pub  fn status_with(&mut self, func: impl FnOnce(&mut crate::api::networking::v1::NetworkPolicyStatus)) -> &mut Self {
        if self.status.is_none() { self.status = Some(Default::default()) };
