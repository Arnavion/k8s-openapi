...
impl IDRange  {
    /// Set [`Self::max`]
    pub  fn max_set(&mut self, max: impl Into<i64>) -> &mut Self {
        self.max = max.into(); self
    }

    pub  fn max(&mut self) -> &mut i64 {
        &mut self.max
    }

    /// Modify [`Self::max`] with a `func`
    pub  fn max_with(&mut self, func: impl FnOnce(&mut i64)) -> &mut Self {
        func(&mut self.max); self
    }


    /// Set [`Self::min`]
    pub  fn min_set(&mut self, min: impl Into<i64>) -> &mut Self {
        self.min = min.into(); self
    }

    pub  fn min(&mut self) -> &mut i64 {
        &mut self.min
    }

    /// Modify [`Self::min`] with a `func`
    pub  fn min_with(&mut self, func: impl FnOnce(&mut i64)) -> &mut Self {
